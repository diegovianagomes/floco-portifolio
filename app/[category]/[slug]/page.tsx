...
export default async function ProjectPage({ params }: PageProps) {
  ...
  const hasValidFloorPlans = project.floorPlans?.some(plan => plan.trim() !== "");

  return (
    <PageLayout>
      <div className="px-8 mt-8 sticky top-0 bg-white  bg-opacity-90  ">
        <div className="flex w-full justify-between mt-4 mb-4 border-b-4 border-[#B7CCE9] ">
          <h1 className="mb-4 mt-4 ml-2 font-bold text-xl text-[#B7CCE9] font-sans antialiased">{project.title}</h1>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full mb-8 p-8">
        <div className="w-full hidden md:block ">
          {/* Intentional empty spacer column for layout purposes */}
        </div>
          ...
          {project.images.map((image: string, index: number) => (
            <div key={index} className="w-full bg-white pl-8 pr-8 pt-4 pb-4">
              <Image
                ...
                alt={`${project.title} - ${project.localizacao} - Image ${index + 1}`}
                width={800}
                height={600}
      ...
      <div className="flex w-full justify-between pl-16 pr-16  border-b-4 border-[#B7CCE9] "></div>
      ...
      <div className="grid grid-cols-2 gap-8 w-full mb-8 mt-8 pl-8 delay-1000">
        <div className="w-full border-d-2">
          ...
          {project.equipe.split('\n').map((member, index) => (
            <div key={`member-${index}`} className="mb-1">{member}</div>
          ))}
          </div>
          ...
          {project.video && (
            <>
              ...
              <p className="font-light text-sm font-sans antialiased pb-2">{project.video}</p>
            </>
          )}
        </div>

        <div className="w-full">
          <p className="font-normal text-sm font-sans antialiased">Status: </p>
